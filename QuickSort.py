...
def sort(x, first, last):
    ## print(str(first) + "→" + str(last))
    if first == last:
        return 0
    delete = 0 # 在这里删除了多少个元素
    com = x[first] # 将要用来比较的元素，比它小的插入在first前面，比它大的插入在last-delete+1前面
    ...
    if smaller > 0: # 因为可能没有更小的了
        a = sort(x, first, first + smaller -1)
    b = 0
    if larger > 0: # 同理
        b = sort(x, first + smaller + 1 - a, first + smaller + larger - a) # 这里会直接受到a的影响
    return a + b + delete # 返回所有在这期间删去的值

x = [1, 1, 3, 3, 2, 2, 1, 5, 4, 4, 5, 6 ,7 , 6, 7, 6, 8, 8, 8, 1, 2, 3, 4, 4, 3, 2, 1, 100]
sort(x, 0 ,len(x) - 1)
print(x)